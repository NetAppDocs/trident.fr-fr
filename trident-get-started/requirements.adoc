...
[]
=====
* Kubernetes 1.36 est désormais pris en charge dans Trident. Mettez à niveau Trident avant de mettre à niveau Kubernetes.
...
* OpenShift 4.12, 4.14 - 4.22 (Si vous prévoyez d'utiliser la préparation de nœud iSCSI avec OpenShift 4.19, la version minimale de Trident prise en charge est 25.06.1.)
+
...
* Systèmes FAS, AFF ou ASA r2 sur site (iSCSI, NVMe/TCP et FC) exécutant des versions ONTAP sous support complet ou limité de NetApp . Voir link:https://mysupport.netapp.com/site/info/version-support["Prise en charge des versions logicielles"].
...
== Prise en charge de Trident pour Kubernetes sur Kubernetes et les plans de contrôle hébergés OpenShift

À partir de la version 26.06, Trident prend pleinement en charge la topologie Kubernetes sur Kubernetes. Les clusters Kubernetes dont les nœuds s’exécutent en tant que machines virtuelles KubeVirt sur un cluster d’infrastructure (hôte). Cette topologie sous-tend les offres de plan de contrôle hébergé telles que les plans de contrôle hébergés (HCP) OpenShift sur OpenShift Virtualization.


NOTE: Cette configuration fonctionnait dans les versions précédentes, mais la qualification complète commence avec 26.06.

Le cluster invité (locataire) utilise le pilote CSI KubeVirt (`kubevirt-csi` comme fournisseur de stockage. Lorsqu'une charge de travail dans le cluster invité requiert un  `PersistentVolumeClaim` volume, le pilote CSI KubeVirt crée un volume correspondant dans le cluster d'infrastructure. Trident provisionne le volume à partir d'ONTAP, et le volume est connecté à chaud à la machine virtuelle du nœud invité. Trident s'exécute uniquement dans le cluster d'infrastructure. Le cluster invité consomme le stockage ONTAP indirectement via le pilote CSI KubeVirt.

.Exigences :
* Trident est installé sur le cluster d'infrastructure avec un backend ONTAP pris en charge.
* Une classe de stockage Trident existe dans le cluster d'infrastructure pour que le pilote CSI KubeVirt puisse mapper les volumes du cluster invité.
* KubeVirt ou OpenShift Virtualization est déployé sur le cluster d'infrastructure, et le pilote CSI KubeVirt est installé dans chaque cluster invité.
...
| Kubernetes (infrastructure et cluster invité) | 1,36 


| KubeVirt | 1.9.0-beta.0 


| Importateur de données conteneurisé (CDI) | 1,65 
|===
.Points à prendre en compte :
* Dans le cluster invité, les volumes apparaissent comme des périphériques de stockage par blocs connectés à chaud sur les machines virtuelles des nœuds invités.
* Le flux de travail du pilote CSI KubeVirt ne nécessite pas Trident dans le cluster invité. Vous configurez le backend ONTAP et la classe de stockage dans le cluster d'infrastructure.


.Installez Trident directement dans le cluster invité :
Vous pouvez également installer Trident directement dans le cluster invité (tenant). Installez Trident dans le cluster invité lorsque vous avez besoin de l'ensemble des fonctionnalités de Trident, telles que les instantanés, les clones, le redimensionnement de volumes et l'importation, que  `kubevirt-csi` ne fournit pas à lui seul.

Dans le cluster invité, utilisez les protocoles IP suivants : NFS, iSCSI et NVMe/TCP. Le cluster invité ne prend pas en charge Fibre Channel (FC) car les nœuds invités sont des machines virtuelles sans connectivité FC directe.
...
=== Images de conteneur requises pour Trident 26.02
...
* docker.io/netapp/trident-operator:26.02.0 (facultatif)
