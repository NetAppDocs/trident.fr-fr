...
Netapp Trident permet la consommation et la gestion des ressources de stockage sur toutes les plateformes de stockage NetApp populaires, dans le cloud public ou sur site, y compris les clusters ONTAP sur site (AFF, FAS et ASA), ONTAP Select, Cloud Volumes ONTAP, Element software (NetApp HCI, SolidFire), Azure NetApp Files, Amazon FSx for NetApp ONTAP et Google Cloud NetApp Volumes.
...
== Plateformes Kubernetes prises en charge

Trident est compatible avec un large éventail de distributions et de plateformes Kubernetes.

Les plateformes prises en charge incluent : * Kubernetes en amont * Red Hat OpenShift * SUSE Harvester 1.7.0 (ONTAP iSCSI)
