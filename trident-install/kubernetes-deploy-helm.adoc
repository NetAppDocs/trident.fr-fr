...
----
. Utilisez  `helm install` et spécifiez un nom pour votre déploiement comme dans l'exemple suivant où  `100..0` est la version de Trident que vous installez.
+
[listing]
----
...
| `enableConcurrency`  a| 
Permet le fonctionnement simultané des contrôleurs Trident pour un débit amélioré. Pour plus d'informations sur les pilotes pris en charge, consultez le lien : https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html#controller-scalability[Controller Scalability documentation].
| faux 
