...
== Informations essentielles concernant Trident 26.06
...
----
. Utilisez `helm install` et spécifiez un nom pour votre déploiement et l'emplacement du registre d'images. Votre link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Images Trident et CSI"] peut être situé dans un registre ou dans des registres différents, mais toutes les images CSI doivent être situées dans le même registre. Dans les exemples,  `100.2602.0` correspond à la version de Trident que vous installez.
+
[role="tabbed-block"]
====
