...
Avant de créer un objet AppVault, vous devez vous assurer que AppVault et le Data Mover que vous choisissez peuvent s'authentifier auprès du fournisseur et des ressources associées.



=== Mots de passe du référentiel de Data Mover

Lorsque vous créez des objets AppVault à l'aide de CRS ou du plug-in de l'interface de ligne de commande Trident Protect, vous pouvez éventuellement demander à Trident Protect d'utiliser un code secret Kubernetes contenant des mots de passe personnalisés pour le chiffrement du référentiel Restic et Kopia. Si vous ne spécifiez pas de secret, Trident Protect utilise les `password` deux mots de passe.

* Lorsque vous créez manuellement AppVault CRS, vous pouvez utiliser le champ *spec.properties.dataMoverPasswordSecretRef* pour spécifier le secret.
* Lorsque vous créez des objets AppVault à l'aide de l'interface de ligne de commande Trident Protect, vous pouvez utiliser `--data-mover-password-secret-ref` l'argument pour spécifier le secret.
...
----
--
.Utilisez l'interface de ligne de commande Trident Protect
--
[source, console]
----
...
* Pour les objets S3 AppVault, vous pouvez éventuellement spécifier un sessionToken, ce qui est utile si vous utilisez la connexion unique (SSO) pour l'authentification. Ce jeton est créé lorsque vous générez des clés pour le fournisseur dans <<Exemples de génération de clés AppVault pour les fournisseurs cloud>>.
