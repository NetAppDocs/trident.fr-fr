...
Si vous prévoyez d'utiliser des https://kubevirt.io/["KubeVirt"^] machines virtuelles avec la réplication SnapMirror, vous devez configurer la virtualisation pour pouvoir geler et annuler le gel de vos SVM. Après avoir configuré la virtualisation, les SVM que vous déployez incluent les outils nécessaires pour geler et débloquer. Pour en savoir plus sur la configuration de la virtualisation, reportez-vous à https://docs.openshift.com/container-platform/4.17/virt/install/installing-virt.html["Installation d'OpenShift Virtualization"^]la section .
