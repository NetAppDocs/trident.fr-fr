...
** *spec.runImmediately* : (_Optionnel_) Définissez sur  `true` pour déclencher une exécution de base unique et immédiate (sauvegarde et/ou snapshot selon les paramètres de rétention) lors de la création de la planification. La valeur par défaut est  `false`. Cela ne modifie pas la récurrence ultérieure.
+
