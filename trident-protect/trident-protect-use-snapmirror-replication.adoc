...
NOTE: Lors de l'utilisation d'une modification commerciale (CR), créez manuellement l'espace de noms de destination avant d'appliquer la CR. Trident Protect crée automatiquement des espaces de noms uniquement lors de l'utilisation de l'interface de ligne de commande (CLI).
...
*** *Spécifications storageClassMapping* : (Optionnel) Associe les classes de stockage source aux classes de stockage de destination pour chaque PVC au sein du même AMR. Chaque entrée nécessite un `source` et un `destination` nom de classe de stockage. Lorsqu'elle est définie, `storageClassMapping` remplace `storageClassName` entièrement ; les PVC sans entrée source correspondante utilisent la classe de stockage par défaut du cluster, et non `storageClassName`.
+
Lors de l'utilisation  `storageClassMapping` dans un CR AppMirrorRelationship, veuillez prendre en compte les restrictions suivantes :

+
**** `storageClassMapping`remplace `storageClassName`entièrement. Lorsque les deux champs sont renseignés, `storageClassName`est ignoré.
**** Les PVC avec des classes de stockage source qui n'ont pas d'entrée correspondante dans `storageClassMapping` utilisent la classe de stockage par défaut du cluster, pas `storageClassName`.
**** Chaque entrée nécessite à la fois une valeur  `source` et  `destination`.
**** Modifier la classe de stockage de destination peut entraîner la suppression et la recréation des PVC de destination existants.
...
NOTE: Utilisez  `--storageclass-mapping` pour associer des classes de stockage source individuelles à des classes de stockage de destination. Spécifiez une ou plusieurs  `source:destination` paires séparées par des virgules (par exemple,  `"class-a:class-b,class-c:class-d"`). Utilisez  `*:destination-class` comme caractère générique pour associer tous les PVC non mappés à une seule classe de stockage de destination. Lorsque  `--storageclass-mapping` est défini,  `--storage-class` est ignoré.

+
*Exemple utilisant une seule classe de stockage de destination (`--storage-class`):*
