...
NOTE: Pour définir une application à portée de machine virtuelle qui protège des machines virtuelles KubeVirt spécifiques par leur nom plutôt que des espaces de noms entiers, utilisez le champ  `spec.includedVirtualMachines` dans la ressource personnalisée (Application CR) ou le paramètre  `--virtual-machines` dans l’interface de ligne de commande (CLI). Pour plus de détails, la référence des champs et des exemples YAML, consultez link:trident-protect-kubevirt-vms.html["Protégez les machines virtuelles KubeVirt avec Trident Protect"].
...
. (_Facultatif_) Si nécessaire, vous pouvez ajouter un filtrage des ressources à la même CR pour inclure ou exclure des ressources spécifiques :
+
...
----
Pour définir une application utilisant uniquement des PVC, vous devez également inclure  `PersistentVolume` et  `VolumeSnapshotClass`` dans le filtre de ressources. Les opérations de snapshot et d'archivage des sauvegardes dépendent de  `PersistentVolume` (le volume à l'échelle du cluster lié à chaque PVC) et de  `VolumeSnapshotClass` (le pilote de snapshot), et échoueront sans eux. Par exemple :
