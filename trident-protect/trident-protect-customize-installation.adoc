...
Vous pouvez personnaliser les paramètres AutoSupport, le filtrage des espaces de noms et la gestion des certificats webhook OpenShift pour répondre à vos besoins spécifiques. Le tableau suivant décrit les paramètres de configuration disponibles :
...
| useOpenshiftServiceCA | booléen | Permet l'intégration avec l'opérateur d'autorité de certification de service natif de OpenShift pour la création et l'injection automatiques de certificats TLS pour le webhook d'admission Trident Protect. Lorsqu'il est défini sur  `true`, l'opérateur d'autorité de certification de service gère automatiquement les certificats TLS du webhook. À utiliser uniquement sur les clusters OpenShift. (par défaut :  `false`) 
|===
