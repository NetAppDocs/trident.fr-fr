...
Les étapes décrites dans cette section expliquent comment créer le backend séparément. Pour savoir comment utiliser la fonctionnalité de backend automatique, reportez-vous à la section link:trident-fsx-storageclass-pvc.html#configure-a-kubernetes-storageclass-object-with-automatic-backend-configuration-support-for-fsx-for-ontap["Configurer StorageClass et PVC"] pour plus d'informations.
...
Vous pouvez provisionner des volumes SMB à l'aide du  `ontap-nas` driver. Avant de terminer link:trident-fsx-storage-backend.html#ontap-san-and-nas-driver-integration["Intégration des pilotes ONTAP SAN et NAS"], complétez ces étapes : link:worker-node-prep.html#prepare-to-provision-smb-volumes.html["Préparez-vous au provisionnement des volumes SMB"].
