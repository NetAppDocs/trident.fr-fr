...
Avant de pouvoir provisionner des volumes avec Trident, vous devez intégrer votre stockage backend à Trident en créant une Trident Backend Configuration (TBC). La TBC définit comment Trident se connecte au stockage backend et fournit les informations nécessaires pour que Trident puisse gérer les ressources de stockage.



[[create-a-trident-backend-configuration-tbc-for-fsx-for-ontap]]
== Créer une configuration de backend Trident (TBC) pour FSx for ONTAP

Il existe deux façons de configurer le backend Trident pour utiliser un système de fichiers FSx for ONTAP : * Créer un StorageClass qui exploite la fonctionnalité de backend automatique. * Créer le backend et le StorageClass séparément.

Les étapes décrites dans cette section expliquent comment créer le backend séparément. Pour savoir comment utiliser la fonctionnalité de backend automatique, reportez-vous à la section link:trident-fsx-storageclass-pvc.html#configure-a-storage-class-without-automatic-backend-configuration["Configurer StorageClass et PVC"] pour plus d'informations.
...
| `storageDriverName` | Nom du pilote de stockage. Ne peut pas être modifié après création. Pour mettre à jour ce paramètre, vous devez créer un nouveau backend. | `ontap-nas`, `ontap-nas-economy`, `ontap-nas-flexgroup`, `ontap-san`, `ontap-san-economy` 
...
| `storagePrefix` | Préfixe utilisé pour le provisionnement des nouveaux volumes dans la SVM. Ne peut pas être modifié après sa création. Pour mettre à jour ce paramètre, vous devez créer un nouveau backend. | `trident` REMARQUE : Les paramètres de configuration du backend qui affectent l'identité du backend et la dénomination des volumes, tels que  `storageDriverName` et  `storagePrefix`, ne peuvent pas être modifiés après leur création. Pour les mettre à jour, vous devez créer un nouveau backend. 
...
Vous pouvez provisionner des volumes SMB à l'aide du  `ontap-nas` driver. Avant de terminer link:trident-fsx-storage-backend.html#fsx-for-ontap-driver-details["Intégration des pilotes ONTAP SAN et NAS"], complétez ces étapes : link:worker-node-prep.html#prepare-to-provision-smb-volumes["Préparez-vous au provisionnement des volumes SMB"].
