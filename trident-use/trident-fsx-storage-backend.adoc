...
Les étapes décrites dans cette section expliquent comment créer le backend séparément. Pour savoir comment utiliser la fonctionnalité de backend automatique, reportez-vous à la section xref:trident-fsx-storageclass-pvc.html#configure-a-kubernetes-storageclass-object-with-automatic-backend-configuration-support-for-fsx-for-ontap[Configurer StorageClass et PVC] pour plus d'informations.
...
Vous pouvez provisionner des volumes SMB à l'aide de `ontap-nas` conducteur. Avant de terminer <<ONTAP SAN and NAS driver integration>> Suivez ces étapes : link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["Préparez-vous au provisionnement des volumes SMB"].
