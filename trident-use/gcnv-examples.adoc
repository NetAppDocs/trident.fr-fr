...
| `snapshotDir` | Accès au `.snapshot` répertoire | `true`, `false` (Le comportement par défaut peut varier. À définir explicitement) « false » pour NFSv3 
...
NOTE: Remplacez  `<id_value>` et  `<key_value>` par vos identifiants de compte de service.
...
[%collapsible]
====
Lors de l'installation de Trident avec une identité cloud GKE, les backends s'authentifient en tant qu'identité de charge de travail et omettent le `apiKey` bloc. L'identité cloud est configurée de la même manière pour les deux pilotes Google Cloud NetApp Volumes. La seule différence concerne la valeur `storageDriverName` : utilisez `google-cloud-netapp-volumes` pour les volumes NAS (NFS ou SMB) et `google-cloud-netapp-volumes-san` pour les volumes SAN (bloc iSCSI).


NOTE: La prise en charge de l'identité cloud pour le  `google-cloud-netapp-volumes-san` driver nécessite Trident 26.06 ou une version ultérieure. Les volumes SAN sont provisionnés à partir de pools de stockage Flex Unified, donc définissez  `serviceLevel` sur  `flex`.

.Pilote NAS (<code>google-cloud-netapp-volumes</code>)
[source, yaml]
----
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
...
----
.Pilote SAN (<code>google-cloud-netapp-volumes-san</code>)
[source, yaml]
----
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
