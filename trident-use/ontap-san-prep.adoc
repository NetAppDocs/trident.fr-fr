...
À partir de la version 25.02, Trident détecte automatiquement le profil ASA r2 de ONTAP 9.16.1 ou version ultérieure et permet le provisionnement et l'utilisation du stockage sur ASA r2 via le protocole iSCSI.
...
NOTE: Trident exige qu'un ou plusieurs agrégats soient affectés au SVM. Lors de l'utilisation de ASA r2, cette action n'est disponible qu'en mode via l'interface de ligne de commande de ONTAP `diag`.
