...
'''
sidebar: sidebar permalink: trident-use/ontap-nas-examples.html keywords: map backends, storage classes, trident backend, ontap-nas, ontap-nas-economy, ontap-nas-flexgroups, options, storageclasses, fsx, metrocluster summary: Découvrez comment créer et utiliser les pilotes ONTAP NAS avec votre installation Trident. Cette section fournit des exemples de configuration de backend et des détails sur la façon de mapper les backends aux StorageClasses. ---
