...
summary: Découvrez comment créer et utiliser des pilotes NAS ONTAP avec votre installation Trident. Cette section fournit des exemples de configuration de backend et des détails sur la façon de mapper les backends aux StorageClasses. 
---
...
[role="lead"]
Créez et utilisez des pilotes NAS ONTAP avec votre installation Trident. Cette section fournit des exemples de configuration de backend et des détails sur le mappage des backends vers les StorageClasses. À partir de la version 25.10, NetApp Trident prend en charge link:https://docs.netapp.com/us-en/ontap-afx/index.html["Systèmes de stockage NetApp AFX"^]. Les systèmes de stockage NetApp AFX diffèrent des autres systèmes basés sur ONTAP (ASA, AFF et FAS) par l’implémentation de leur couche de stockage.
...
À partir de la version 26.06, vous pouvez activer le placement équilibré ONTAP afin que ONTAP sélectionne l’agrégat pour les nouveaux volumes. Le placement équilibré est requis pour les systèmes de stockage AFX sur ONTAP 9.19.1 ou version ultérieure. Consultez link:ontap-balanced-placement.html["Utilisez un placement équilibré avec les backends ONTAP"] pour connaître les exigences et obtenir des exemples.
...
Ne peut pas être modifié après la création. Pour mettre à jour ce paramètre, vous devez créer un nouveau backend.
...
NOTE: Lors de l'utilisation d'ontap-nas-economy et d'un storagePrefix de 24 caractères ou plus, les qtrees n'intègrent pas le préfixe de stockage, bien qu'il soit présent dans le nom du volume.
| « trident » 
...
NOTE: Lorsqu'un agrégat est mis à jour dans SVM, il est automatiquement mis à jour dans Trident par interrogation de SVM, sans qu'il soit nécessaire de redémarrer le contrôleur Trident. Si vous avez configuré un agrégat spécifique dans Trident pour provisionner des volumes, et que cet agrégat est renommé ou déplacé hors de la SVM, le backend passe en état d'échec dans Trident lors de l'interrogation de l'agrégat SVM. Vous devez soit remplacer l'agrégat par un agrégat présent sur la SVM, soit le supprimer complètement pour remettre le backend en ligne.
...
*Si spécifié, définissez toujours sur  `true` pour les systèmes de stockage AFX sur ONTAP 9.19.1 ou version ultérieure.*
| `false`; `true` pour les systèmes de stockage AFX sur ONTAP 9.19.1 ou version ultérieure 
...
NOTE: Lorsque la liste d’agrégats est mise à jour dans SVM, la liste est automatiquement mise à jour dans Trident par interrogation de SVM, sans qu’il soit nécessaire de redémarrer le contrôleur Trident. Lorsque vous avez configuré une liste d’agrégats spécifique dans Trident pour le provisionnement de volumes, si la liste d’agrégats est renommée ou déplacée hors de SVM, le backend passe à l’état d’échec dans Trident lors de l’interrogation de l’agrégat SVM. Vous devez soit modifier la liste d’agrégats pour en utiliser une présente sur SVM, soit la supprimer complètement afin de remettre le backend en ligne.
| « » 
...
Les backends existants issus d’installations précédentes provisionnent les volumes comme expliqué ci-dessus lors de la mise à niveau de Trident. Pour les volumes que vous avez créés avant la mise à niveau, vous devez redimensionner ces volumes pour que la modification soit prise en compte. Par exemple, un PVC de 2 Gio avec `snapshotReserve=50` aboutissait auparavant à un volume offrant 1 Gio d’espace accessible en écriture. En redimensionnant le volume à 3 Gio, par exemple, l’application disposera de 3 Gio d’espace accessible en écriture sur un volume de 6 Gio.
...
Les définitions de StorageClass suivantes font référence à <<Exemples de systèmes back-end avec pools virtuels>>. En utilisant le champ  `parameters.selector`, chaque StorageClass indique quels pools virtuels peuvent être utilisés pour héberger un volume. Le volume possède les aspects définis dans le pool virtuel choisi.

* Le  `protection-gold` StorageClass correspond aux première et deuxième pools virtuelles dans le  `ontap-nas-flexgroup` backend. Ce sont les seuls pools offrant une protection de niveau or.
+
[source, yaml]
----
...
parameters:
  ...
  fsType: "ext4"
----
* Le  `protection-not-gold` StorageClass correspond au troisième et au quatrième pool virtuel dans le  `ontap-nas-flexgroup` backend. Ce sont les seuls pools offrant un niveau de protection autre que gold.
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* Le  `app-mysqldb` StorageClass correspond au quatrième pool virtuel dans le  `ontap-nas` backend. C'est le seul pool offrant une configuration de pool de stockage pour les applications de type mysqldb.
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* Le  `protection-silver-creditpoints-20k` StorageClass correspond au troisième pool virtuel dans le  `ontap-nas-flexgroup` backend. Il s'agit du seul pool offrant une protection de niveau argent et 20000 points de crédit.
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* Le  `creditpoints-5k` StorageClass correspond au troisième pool virtuel dans le  `ontap-nas` backend et au deuxième pool virtuel dans le  `ontap-nas-economy` backend. Ce sont les seules offres de pool avec 5000 points de crédit.
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
