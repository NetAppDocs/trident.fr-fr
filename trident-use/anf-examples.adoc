...
| `backendName` | Nom personnalisé pour le backend de stockage | Nom du pilote + "_" + caractères aléatoires 
...
== Envisagez les environnements cloud Azure (26.06)

À partir de la version 26.06, Trident prend en charge la création et la gestion de backends Azure NetApp Files dans plusieurs environnements cloud Azure.
...
NOTE: Si le réseau virtuel est situé dans un groupe de ressources différent de celui du compte de stockage Azure NetApp Files (ANF), spécifiez le groupe de ressources du réseau virtuel lors de la configuration de la liste des groupes de ressources pour le backend.
...
| `snapshotDir` | Accès au `.snapshot` répertoire | `true`, `false` (Défini explicitement). 
...
NOTE: `nasType: smb`Filtres pour les pools prenant en charge les volumes SMB.
