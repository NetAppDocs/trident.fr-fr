...
. Sur le cluster Kubernetes principal, créez le TridentTridentSnapshotInfoCR pour obtenir les détails internes.
+
.Exemple
[source, yaml]
----
...
----
+
.. Sur le cluster Kubernetes secondaire, mettez à jour le champ _spec.state_ de la ressource personnalisée _TridentMirrorRelationship_ à _promoted_ et _spec.volumeMappings[].promotedSnapshotHandle_ pour qu’il corresponde à l’internalName du snapshot.
