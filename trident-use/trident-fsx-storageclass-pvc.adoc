...
[role="lead"]
La  https://kubernetes.io/docs/concepts/storage/storage-classes/["Objet classe de stockage Kubernetes"^] identifie un provisionneur et lui indique comment provisionner les volumes. Cette section montre comment configurer un objet Kubernetes StorageClass qui spécifie Trident comme provisionneur.
...
NOTE: Si vous souhaitez configurer manuellement le stockage backend, veuillez vous référer à la section <<create-a-kubernetes-storageclass-without-automatic-backend-configuration>> pour savoir comment créer séparément le backend Trident et la classe de stockage.
...
Vous pouvez transmettre des paramètres backend optionnels via le StorageClass. Définissez toutes les valeurs optionnelles sous forme de chaînes dans la section StorageClass `parameters`. Pour une liste complète des paramètres du backend, voir : link:trident-fsx-storage-backend.html#backend-advanced-configuration-and-examples["Configuration du backend FSx for NetApp ONTAP"].
...
Pour plus d'informations sur les autres options de configuration, consultez la <<first-link,Configuration avancée du backend et exemples>> section ci-dessous.
...
Vous pouvez provisionner des volumes SMB à l'aide du  `ontap-nas` driver. Cependant, pour ce faire, vous devez effectuer ces étapes : link:worker-node-prep.html#prepare-to-provision-smb-volumes["Préparez-vous au provisionnement des volumes SMB"].



[[first-linkbackend-advanced-configuration-and-examples]]
== [[first-link]]Configuration avancée et exemples du backend
