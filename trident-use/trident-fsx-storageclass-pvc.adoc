...
Vous pouvez transmettre des paramètres backend optionnels via le StorageClass. Définissez toutes les valeurs optionnelles sous forme de chaînes dans la section StorageClass `parameters`. Pour une liste complète des paramètres du backend, voir : xref:trident-fsx-storage-backend.html#backend-advanced-configuration-and-examples[Configuration du backend FSx for NetApp ONTAP].
...
Vous pouvez provisionner des volumes SMB à l'aide du  `ontap-nas` driver. Cependant, pour ce faire, vous devez effectuer ces étapes : link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["Préparez-vous au provisionnement des volumes SMB"].
