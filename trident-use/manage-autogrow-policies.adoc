...
[role="lead"]
Une fois les stratégies d'extension automatique créées, vous pouvez les consulter, les modifier et les supprimer. Vous pouvez également surveiller les volumes qui utilisent une stratégie donnée.
...
IMPORTANT: Les modifications affectent tous les volumes utilisant cette stratégie. Dans la mesure du possible, testez les modifications d'abord dans un environnement hors production.
...
* *Effet immédiat :* Tous les volumes qui utilisent cette politique adoptent les nouveaux paramètres lors de la prochaine évaluation de croissance.
* *Aucun redémarrage du volume requis :* Les modifications s’appliquent à la prochaine opération de croissance.
...
Les politiques d'extension automatique utilisent la protection du finaliseur pour empêcher toute suppression accidentelle pendant que les volumes les utilisent activement.
...
Autogrow prend en charge les backends ONTAP. Autogrow prend également en charge Google Cloud NetApp Volumes (GCNV) pour les volumes NAS et SAN.
...
Définissez l'annotation PVC sur  `"none"`. Ce paramètre remplace toute stratégie de niveau StorageClass pour ce volume :
...
Trident cesse d'étendre le volume. Il ne crée plus de demandes de redimensionnement pour ce volume, même si l'utilisation continue d'augmenter au-delà de `usedThreshold`.

Par exemple, avec cette politique, Trident cesse d'augmenter le volume une fois qu'il atteint 100 Gio :
...
Oui. Lorsque vous mettez à jour une stratégie, tous les volumes qui utilisent cette stratégie adoptent les nouveaux paramètres lors de la prochaine évaluation de la croissance. Aucun redémarrage de volume n'est requis.
...
Les politiques utilisent une protection de finalisation. Si des volumes utilisent encore la politique, la suppression entre dans un état  `Deleting` et attend que vous retiriez tous les volumes de la politique.
...
kubectl annotate pvc <pvc-name> \
  trident.netapp.io/autogrowPolicy-
----
Après que vous avez supprimé tous les volumes, Trident libère le finaliseur et supprime la stratégie.



=== Quels backends sont pris en charge par autogrow ?

Autogrow prend en charge les backends ONTAP. Il prend également en charge Google Cloud NetApp Volumes (GCNV) pour les volumes NAS et SAN.

La fonctionnalité d'extension automatique prend en charge les protocoles NFS, iSCSI, FCP et NVMe. Les volumes de blocs bruts NVMe nécessitent ONTAP 9.16.1 ou une version ultérieure.

Dans les déploiements brownfield, les volumes existants peuvent nécessiter la migration de la publication du volume avant que l’extension automatique ne prenne effet. Pour vérifier l’état de la migration, consultez les journaux du contrôleur Trident :
