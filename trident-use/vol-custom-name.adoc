...
* Dans le cas du `ontap-nas-economy` pilote, seul le nom du volume Qtree est conforme au modèle de nom.
* Dans le cas du `ontap-san-economy` pilote, seul le nom du LUN est conforme au modèle de nom.
...
* Les étiquettes personnalisées ne sont prises en charge que pour les `ontap-san`, `ontap-nas`, et `ontap-nas-flexgroup` conducteurs.
