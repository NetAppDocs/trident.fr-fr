...
* La prise en charge de la concurrence pour les pilotes ONTAP-NAS (NFS uniquement), ONTAP-SAN (iSCSI, FCP, NVMe) et Google Cloud NetApp Volumes est désormais généralement disponible (GA).
* Ajout de la prise en charge de la configuration automatique du backend pour Amazon FSx for NetApp ONTAP. Lorsque vous créez un StorageClass qui inclut les paramètres requis, NetApp Trident crée automatiquement le backend correspondant et la VolumeSnapshotClass (si nécessaire).
* Ajout de la prise en charge de différents clouds Microsoft Azure, tels qu'Azure Government et Azure China, et d'une configuration cloud personnalisée pour Azure NetApp Files backends.
* Évolutivité améliorée du contrôleur pour les volumes NVMe.
* Ajout de la prise en charge de la fonctionnalité d'autogrow de volume. Vous pouvez contrôler le comportement d'autogrow en définissant des règles d'autogrow personnalisées NetApp Trident.
* Ajout de la prise en charge de Google Cloud NetApp Volumes stockage bloc (SAN/iSCSI).
* Ajout de la prise en charge de la hiérarchisation automatique pour Google Cloud NetApp Volumes. Vous pouvez désormais optimiser les coûts de stockage en ajoutant des contrôles natifs Kubernetes pour configurer la hiérarchisation automatique FabricPool d'ONTAP pour Google Cloud NetApp Volumes via NetApp Trident.
* Ajout de la prise en charge de Kubernetes 1.35.
