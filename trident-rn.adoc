...
* *Kubernetes* :
+
** Ajout de la prise en charge des instantanés de groupe de volumes CSI avec  `v1beta1` API Kubernetes d'instantané de groupe de volumes pour le pilote iSCSI ONTAP-SAN. Voir link:../trident-use/vol-group-snapshots.html["Travailler avec des instantanés de groupes de volumes"^].
** Prise en charge ajoutée pour ONTAP ASA r2 pour NVMe/TCP en plus d'iSCSI. Voir link:../trident-use/ontap-san-examples.html["Options et exemples de configuration des SAN ONTAP"^].
** Prise en charge SMB sécurisée ajoutée pour les volumes ONTAP-NAS et ONTAP-NAS-Economy. Les utilisateurs et groupes Active Directory peuvent désormais être utilisés avec les volumes SMB pour une sécurité renforcée. Voir link:../trident-use/ontap-nas-prep.html#enable-secure-smb["Activer le SMB sécurisé"^].
...
** Ajout de la prise en charge de la réplication de volume avec SnapMirror pour les backends NVMe. Voir link:../trident-use/vol-volume-replicate.html["Réplication de volumes à l'aide de SnapMirror"^].
