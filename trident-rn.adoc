...
** Ajout d'une option permettant de diffuser les métriques Prometheus via HTTPS au lieu de HTTP. Voirlink:../trident-use/monitor-trident.html["Surveillez Trident"^] .
** Ajout d'une option `--no-rename` lors de l'importation d'un volume pour conserver le nom d'origine mais laisser Trident gérer le cycle de vie du volume. Voirlink:../trident-use/vol-import.html["Importer des volumes"^] .
