...
== Nouveautés de la version 26.06.1

Découvrez les nouveautés de NetApp Trident et Trident Protect, y compris les améliorations, les correctifs et les suppressions.
...
* *Kubernetes* :
+
** Correction d'un problème de concurrence entre la publication CSI et le déplacement de volume qui pouvait permettre d'attacher un volume en utilisant des informations d'accès obsolètes.
** Correction d'un problème de concurrence où NodeGetInfo pouvait renvoyer des étiquettes de topologie vides avant la fin de l'enregistrement du nœud, ce qui entraînait des échecs de provisionnement tenant compte de la topologie.
** Correction d'un problème rare de concurrence de déconnexion NVMe qui pouvait interrompre un sous-système partagé alors qu'un autre pod était encore en cours de connexion.
** Correction des échecs de publication de clones PVC NVMe (et du mappage incorrect de l'espace de noms parent) lors de la création de clones d'un volume monté en situation de concurrence.
** Correction des échecs de création d'instantanés pour les PVC sur les backends ONTAP-SAN-Economy utilisant un nameTemplate.
...
* *Kubernetes* :
+
** Ajout de l’option  `storageAPITimeout` (secondes) dans la configuration du backend ONTAP pour définir les délais d’expiration des clients API REST et ZAPI par backend.
...
[[whats-new-in-26-06]]
== Quoi de neuf dans la version 26.06
...
* *Kubernetes* :
+
** Ajout de la prise en charge de la disponibilité générale (GA) pour la concurrence des pilotes ONTAP-NAS-Economy et ONTAP-SAN-Economy, faisant passer cette fonctionnalité de la version d'évaluation technique à la version finale, et ajout de la prise en charge de la concurrence GA pour les pilotes Azure NetApp Files (ANF) et SolidFire.
** Ajout de la prise en charge de Kubernetes 1.36.
** Ajout de la prise en charge de la disponibilité générale (GA) pour l’extension automatique des volumes avec les pilotes NAS et SAN Google Cloud NetApp Volumes (GCNV).
** Ajout du placement équilibré Unified ONTAP pour une sélection optimale des agrégats lors du provisionnement de nouveaux volumes (link:https://github.com/NetApp/trident/issues/64["Problème n° 64"]).
** Ajout de la prise en charge ONTAP AFX pour le pilote ONTAP-NAS-Flexgroup dans ONTAP.
** Ajout d'une intégration avec le NetApp Shift Toolkit pour simplifier les migrations de machines virtuelles lors de l'utilisation du Red Hat OpenShift Migration Toolkit for Virtualization.
** Ajout de la prise en charge de Cloud Identity pour le pilote SAN GCNV.
** Ajout de la prise en charge des volumes subordonnés pour la propagation de la publication de volumes Trident (TVP) et les flux de travail d’extension automatique.
** Ajout de la prise en charge des mises à jour de volumes et des instantanés de groupes de volumes lors de l’exécution de Trident avec la concurrence activée.
** Ajout d'une limitation dynamique du débit gRPC pour GCNV.
** Ajout d'une sonde d'exécution de disponibilité du contrôleur et optimisation des paramètres QPS et Burst de l'API Kubernetes par défaut pour le contrôleur et les sidecars.
** Amélioration du séquencement des opérations sur les nœuds grâce au découplage des étapes d'attachement, de configuration du chiffrement et de formatage du système de fichiers.
** Les systèmes de fichiers racine des conteneurs Trident ont été montés en lecture seule pour améliorer la sécurité.
** Ajout d'une suppression au mieux des instantanés pour les instantanés de volumes clonés dans les backends ONTAP lorsque l'instantané source d'un volume cloné est supprimé.
** Ajout de la reconnaissance des PVC de clone de sauvegarde pour empêcher les PVC temporaires créés par Veeam Kasten, Trilio et Cohesity de rester bloqués dans la file d'attente de récupération de volume ONTAP.
** Gestion améliorée des ARN AWS pour la prise en charge de régions spécifiques.
** Empêcher le rétrécissement des Flexvols lors de l'opération de redimensionnement pour le pilote ONTAP-SAN-ECONOMY.
...
* *[Aperçu technique]* : Ajout de la prise en charge de l'importation de qtree ONTAP-NAS-Economy.
* *[Aperçu technique]* : Ajout de la prise en charge de la concurrence pour les pilotes ASA-R2 (SAN et NVMe).
* *[Aperçu technique]* : Ajout du transport HTTP GCNV en mode ONTAP pour les pilotes ONTAP-SAN et ONTAP-NAS.
...
* *Kubernetes* :
+
** Correction des conditions de concurrence et de la gestion d'état pour les opérations simultanées de publication/dépublication, de clonage, de mise en cache et de mise à jour du backend.
** Correction des conditions de concurrence des règles d'export et de la gestion simultanée de la publication/dépublication pour les volumes subordonnés et les clones en lecture seule dans les pilotes ONTAP-NAS et ONTAP-NAS-Economy.
** Correction de la découverte ONTAP-NAS-Economy FlexVol après un basculement MetroCluster lorsque les règles de snapshot diffèrent (link:https://github.com/NetApp/trident/issues/1082["Problème n° 1082"]).
** Correction des tentatives de suppression de volume ONTAP-NAS-Economy qui interrompaient les suppressions qtree de longue durée (link:https://github.com/NetApp/trident/issues/1121["Problème n° 1121"]).
** Correction du comportement d'importation et de redimensionnement d'ONTAP-SAN et d'ONTAP-SAN-Economy, y compris la validation de fsType, la gestion des métadonnées de volume et le comportement du mode d'extension automatique.
** Correction d'une condition de concurrence d'espace de noms NVMe/TCP lors de la création simultanée de volumes (link:https://github.com/NetApp/trident/issues/1089["Problème n° 1089"]).
** Correction des échecs de redimensionnement iSCSI qui survenaient lorsque les chemins et les portails disponibles ne correspondaient pas, et amélioration du comportement lors des redimensionnements répétés.
** Correction des échecs de suppression de volumes ONTAP-SAN dus à des conditions d'occupation des volumes.
** Correction de la gestion des erreurs ONTAP REST et amélioration du comportement du délai d'expiration de l'état SVM.
** Correction de la gestion des pools zonaux SAN GCNV et des groupes d'hôtes, normalisation de la conversion des identifiants de volume et normalisation des requêtes de création inférieures à la taille minimale.
** Correction de la sélection du pool de capacité natif GCNV afin d’exclure les pools en mode ONTAP.
** Correction du basculement KubeVirt pour les machines virtuelles utilisant Trident Automated Workload Failover afin d'accélérer le basculement des machines virtuelles.
** Correction des problèmes de contrôle d'accès basé sur les rôles (RBAC) liés à l'installation gérée par l'opérateur, notamment les nœuds ClusterRoles manquants et la réconciliation incorrecte de ClusterRole et ClusterRoleBinding lors des mises à niveau. Cela s'applique aux installations basées sur l'opérateur et sur Helm qui utilisent le chart Trident Operator.
** Correction des différences persistantes GitOps sur le TridentConfigurator et les CRD associés en omettant les valeurs de priorité par défaut et en corrigeant l'indentation du modèle YAML (link:https://github.com/NetApp/trident/issues/915["Problème n° 915"], link:https://github.com/NetApp/trident/issues/1120["Problème n° 1120"]).
** Correction du patching des CRD TridentVolumePublication lors d'une mise à niveau Helm (link:https://github.com/NetApp/trident/issues/1151["Problème n° 1151"]).
** Correction de l'ordre de démarrage CSI en démarrant le service CSI gRPC avant les nouvelles tentatives d'enregistrement des nœuds.
** Correction des superpositions de configuration de clonage SMB sécurisé lors d'opérations simultanées.
** Correction des annotations manquantes sur le pod du contrôleur Trident après une mise à niveau (link:https://github.com/NetApp/trident/issues/1004["Problème n°1004"]).
** Amélioration du provisionnement lent des qtrees sur les grands SVM (link:https://github.com/NetApp/trident/issues/1142["Problème n° 1142"]).
** Correction apportée à ControllerPublish pour utiliser le type de système de fichiers de configuration du volume au lieu d'un type de système de fichiers par défaut incorrect.
** Correction d'un problème où les périphériques dm orphelins pouvaient provoquer des échecs CSI répétés NodeUnstageVolume.
** Amélioration du nettoyage des nœuds obsolètes et des chemins multiples grâce à la suppression des artefacts de chemins multiples fantômes et des ressources TridentNode obsolètes après le nettoyage des nœuds supprimés de manière logique.
...
* Ajout de l'intégration de l'opérateur OpenShift Service CA pour automatiser la gestion des certificats TLS pour le webhook d'admission Trident Protect. Activez-la en définissant  `useOpenshiftServiceCA` sur  `true` dans vos valeurs Helm. Pour en savoir plus, consultez link:trident-protect/trident-protect-customize-installation.html["Personnaliser l'installation de Trident Protect"^].
* Ajout de la prise en charge de la sauvegarde et de la restauration des machines virtuelles KubeVirt sur OpenShift Virtualization. Protégez toutes les machines virtuelles d’un espace de noms ou ciblez des machines virtuelles spécifiques par leur nom, et restaurez sélectivement des machines virtuelles individuelles à partir d’une sauvegarde ou d’un instantané. Pour en savoir plus, consultez link:trident-protect/trident-protect-kubevirt-vms.html["Protégez les machines virtuelles KubeVirt avec Trident Protect"^].
* Prise en charge des transformations de ressources ajoutée, vous permettant d'ajouter, de supprimer et de modifier les attributs des ressources Kubernetes lors du processus de restauration. Pour en savoir plus, consultez link:trident-protect/trident-protect-resource-transformations.html["Modifiez les ressources au fur et à mesure de leur restauration à l'aide de ressources personnalisées"].
* Ajout de la prise en charge de  `storageClassMapping` dans AppMirrorRelationship (AMR), permettant le mappage des classes de stockage pour les PVC individuels au sein d'une même définition AMR. Pour en savoir plus, consultez link:trident-protect/trident-protect-use-snapmirror-replication.html["Répliquez les applications à l'aide de NetApp SnapMirror et Trident Protect."^].
...
* Correction d'un problème de restauration de machines virtuelles inter-clusters qui échouait avec une erreur de webhook d'admission lorsque l'espace de noms de l'application source n'existait pas sur le cluster de destination.
* Correction d'un problème de restauration sélective sur place de machines virtuelles qui échouait lors de la création de l'application en raison d'une incompatibilité de spécifications avec une ressource personnalisée d'application existante ; l'application existante est désormais réutilisée.
* Correction d’un problème où les tâches de restauration sur place de machines virtuelles restaient bloquées dans un état  `Running` lorsque la machine virtuelle et son PersistentVolumeClaim étaient supprimés ; un PersistentVolumeClaim manquant est désormais considéré comme une erreur fatale.
* Correction d’un problème qui déclenchait incorrectement, lors de la modification d’une planification de protection existante, une sauvegarde ou une exécution de snapshot immédiate.
* Correction du problème où le finaliseur d'instantané tentait de récupérer la ressource personnalisée de l'application après sa suppression ; le finaliseur ignore désormais l'opération lorsque l'application est introuvable.
* La capacité de stockage protégée fixe augmente considérablement après une restauration sur place d'une machine virtuelle grâce à l'utilisation d'une copie partielle lors de l'étape de copie PersistentVolumeClaim.
* Correction d'un problème où Trident Protect supprimait l'objet snapshot Kubernetes avant de confirmer la suppression du snapshot sous-jacent dans ONTAP ; le finaliseur attend désormais la  `VolumeSnapshot`suppression.
* Correction d'un problème où les instantanés ne se poursuivaient pas lorsqu'une opération de gel de machine virtuelle échouait ; les opérations de gel et de dégel se poursuivent désormais en cas d'échec.
* Correction d’un problème où les applications de machines virtuelles KubeVirt recréaient incorrectement les PersistentVolumeClaims lors des opérations d’inversion, de resynchronisation inverse et de resynchronisation AppMirrorRelationship.




[[whats-new-in-26-02-1]]
== Nouveautés de la version 26.02.1
...
* *Kubernetes* :
* Correction du comportement SAN de Google Cloud NetApp Volumes (GCNV) pour la prise en charge des pools zonaux Flex Unified pour les charges de travail de stockage bloc (iSCSI). Les configurations zonales et régionales sont désormais prises en charge.
* Correction de la gestion des pools NAS UNIFIED Google Cloud NetApp Volumes (GCNV) en améliorant la création, la validation, la dénomination et la recherche de volumes de secours lorsque  `InternalID` est absent.
* Correction d'un problème où les opérations de redimensionnement de périphériques iSCSI pouvaient échouer lorsque les chemins et portails disponibles ne correspondaient pas.
* Correction d'un problème intermittent où  `blkid` identifiait incorrectement le type de système de fichiers pour les périphériques LUKS, empêchant la création de nouveaux volumes.
* Correction d'un problème où les opérations de publication et de préparation LUKS à grande échelle pouvaient bloquer le contrôleur Trident. Les utilisateurs doivent désormais conserver les phrases de passe LUKS associées aux volumes.
* Correction d'un problème où les partitions iSCSI multipath pouvaient apparaître comme des périphériques fantômes, empêchant les opérations CSI `NodeUnstage` de réussir.
* Correction d'une condition de concurrence d'espace de noms NVMe/TCP lors de la création simultanée de volumes (Issue #1089).
* Correction des opérations de clonage d’instantanés simultanées lorsque les instantanés étaient exécutés dans la même seconde.
* Correction d'un problème empêchant les machines virtuelles KubeVirt utilisant  `dataVolumes` de basculer avec Trident Automated Workload Failover.
* Correction d'une erreur qui se produisait lors de la suppression d'une ressource personnalisée non valide `TridentMirrorRelationship` lors de l'utilisation du noyau concurrent de Trident.
* Correction de l'indentation dans la  `tridentactionmirrorupdates`définition YAML du CRD Kubernetes (Issue #1120).
* Correction de l'appel API de suppression asynchrone de qtree REST (Issue #1121).
* Correction apportée  `ControllerPublish`pour utiliser le type de système de fichiers de configuration du volume au lieu d'un type de système de fichiers par défaut incorrect.
* Correction de la journalisation au démarrage du contrôleur pour signaler correctement l'état des pilotes simultanés.
* Correction de la gestion des backends suspendus dans Trident concurrent.
* Gestion améliorée des ARN AWS pour la prise en charge de régions spécifiques.
...
Mise à jour des dépendances de Trident Protect pour corriger les failles de sécurité :
...
* Exécution Go : link:https://nvd.nist.gov/vuln/detail/CVE-2025-22871["CVE-2025-22871"^]
* Dépendances supplémentaires :link:https://nvd.nist.gov/vuln/detail/CVE-2025-23109["CVE-2025-23109"^]




[[whats-new-in-26-02]]
== Quoi de neuf dans la version 26.02
...
* *Kubernetes* :
+
** Ajout de la prise en charge en disponibilité générale (GA) de la concurrence pour ONTAP-NAS (NFS uniquement), ONTAP-SAN (iSCSI, FCP, NVMe) et Google Cloud NetApp Volumes (GCNV), faisant passer cette fonctionnalité de Tech Preview à la version finale. Voir la link:https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html["Documentation sur la scalabilité du contrôleur"] pour plus d'informations.
** Ajout de la prise en charge de l'extension automatique des volumes dans Trident basée sur des stratégies AutoGrow personnalisées et définies par l'utilisateur dans Trident. Voir le link:https://docs.netapp.com/us-en/trident/trident-use/automatic-volume-expansion.html["Documentation sur l’extension automatique du volume"] pour plus d'informations.
** Amélioration de la concurrence des nœuds Trident pour optimiser l'évolutivité des opérations sur les volumes NVMe. Voir la link:https://docs.netapp.com/us-en/trident/trident-use/trident-nvme-rwx-subsystem-limits.html["Documentation sur les volumes NVMe"] pour plus d'informations.
** Ajout de la prise en charge du hiérarchisation automatique des volumes NAS GCNV via les annotations PVC (`tieringPolicy`, `tieringMinimumCoolingDays`), y compris la sélection des pools et l'héritage des clones. Voir la link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-auto-tiering.html["Configurez la hiérarchisation automatique pour la documentation Google Cloud NetApp Volumes"] pour plus d'informations.
** Ajout de la prise en charge des volumes SAN bloc GCNV (iSCSI) utilisant le  `google-cloud-netapp-volumes-san` driver, y compris le provisionnement, le mappage des groupes d'hôtes par nœud pour l'accès aux LUN et les opérations de clonage à partir de volumes. Voir le link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-block.html["Documentation sur la configuration des blocs Google Cloud NetApp Volumes"] pour plus d'informations.
** Ajout de la prise en charge de la configuration automatique du backend pour Amazon FSx for NetApp ONTAP. Lorsque vous créez un StorageClass qui inclut les paramètres requis, Trident crée automatiquement le backend correspondant et la VolumeSnapshotClass, si nécessaire. Voir le link:https://docs.netapp.com/us-en/trident/trident-use/trident-fsx-auto-backend-configuration.html#add-additional-fsxn-file-systems["Documentation de configuration du backend Amazon FSx for NetApp ONTAP"] pour plus d'informations.
** Ajout de la prise en charge de différents clouds Microsoft Azure, tels qu'Azure Government et Azure China, et d'une configuration cloud personnalisée pour Azure NetApp Files backends. Voir le link:https://docs.netapp.com/us-en/trident/trident-use/anf.html["Documentation de configuration du backend Azure NetApp Files"] pour plus d'informations.
** Ajout de la prise en charge de Kubernetes 1.35. Voir le link:https://docs.netapp.com/us-en/trident/trident-get-started/requirements.html["Documentation des exigences"] pour plus d'informations.
...
* *[Aperçu technique]* : Ajout de la prise en charge de la concurrence pour les pilotes ONTAP-NAS-Economy et ONTAP-SAN-Economy.
...
* *Kubernetes* :
+
** Correction d'un problème où la dépublication d'un clone en lecture seule supprimait les règles d'export du volume source dans les pilotes ONTAP-NAS, ONTAP-NAS-Economy et Google Cloud NetApp Volumes link:https://github.com/NetApp/trident/issues/1086["Problème n°1086"].
** Les images kubectl ont été remplacées par des variantes légères basées sur Alpine afin de prévenir les échecs de récupération suite à la dépréciation des images publiques de Bitnami https://github.com/NetApp/trident/issues/1080["Problème n°1080"].
** Correction de la préservation des annotations du déploiement existant lors de la mise à niveau de Trident link:https://github.com/NetApp/trident/issues/1004["Problème n°1004"].
** Autoriser le clonage entre différentes classes de stockage si les deux classes de stockage pointent vers le même backend link:https://github.com/NetApp/trident/issues/1104["Problème n°1104"].
** Correction des échecs de préparation des nœuds dus aux délais d'attente dans les environnements cloud présentant des latences réseau. Augmentation des valeurs de délai d'attente pour les installations cloud.
** Correction d'un problème lors de la création de LUN qui empêchait la définition de l'attribut de type de système de fichiers lorsque le processus entrait dans un état de nouvelle tentative.
** Correction de la recherche de volume via l'API REST afin d'ignorer l'état du volume, empêchant les faux négatifs lors des requêtes de volume.
** Amélioration de l'efficacité du contrôleur Trident pour le pilote ontap-nas-economy lorsqu'il est utilisé à grande échelle.
** Définir internalID lors de l'importation du LUN dans le pilote ontap-san-economy.
** Augmentation des limites de requêtes Azure Resource Graph pour gérer davantage de sous-réseaux.
** Amélioration des délais d'expiration de la division des clones CSI et ONTAP pour éviter les conditions de concurrence avec certaines applications de sauvegarde link:https://github.com/NetApp/trident/issues/1098["Problème #1098"], link:https://github.com/NetApp/trident/issues/1100["Problème n°1100"].
** Correction de la suppression des messages d'erreur LUKS link:https://github.com/NetApp/trident/issues/1069["Problème n°1069"].
** Correction de la gestion des mappeurs LUKS obsolètes pour les protocoles iSCSI et NVMe. La logique de nettoyage améliorée empêche les échecs de montage dus aux mappeurs de périphériques orphelins.


* Limites d'échelle corrigées pour les volumes RWX NVMe.
* Mise à jour du package OpenTelemetry-Go pour corriger link:https://nvd.nist.gov/vuln/detail/cve-2026-24051["CVE-2026-24051"].
...
* Trident Protect désactive désormais automatiquement les planifications de protection et annule les opérations en cours avant une restauration sur place, puis les réactive une fois la restauration terminée. Pour en savoir plus, consultez link:trident-protect/trident-protect-restore-apps.html["Restaurez les applications à l'aide de Trident Protect"^].
* Ajout du champ  `runImmediately` au CR de planification et du paramètre CLI  `--run-immediately` pour déclencher une sauvegarde ou un instantané immédiat lors de la création de la planification. Pour en savoir plus, consultez link:trident-protect/trident-protect-protect-apps.html#create-a-data-protection-schedule["Créez un calendrier de protection des données"^].
* Ajout de la prise en charge de la spécification d'un nom personnalisé pour l'application restaurée à l'aide du  `destinationApplicationName` champ dans le CR de restauration ou du  `--destination-app-name` paramètre CLI. Pour en savoir plus, consultez link:trident-protect/trident-protect-restore-apps.html["Restaurez les applications à l'aide de Trident Protect"^].
...
* Correction des échecs de restauration causés par la création de pods avant que leurs comptes de service requis ne soient disponibles.
* Correction de  `Roles` et  `RoleBindings` qui étaient ignorés lors des restaurations d'applications.
* Correction du problème où le nom du cluster d'origine ne s'affichait pas dans  `tridentctl-protect get appvaultcontent` la sortie, malgré une configuration correcte.
* Correction des erreurs de restauration de Kopia qui étaient ignorées en raison du manque de  `pipefail` prise en charge.
* Correction des échecs de snapshots et de sauvegardes causés par des filtres de ressources qui excluaient les volumes persistants.
* Correction d'une restauration incorrecte de PVC dans les applications multi-espaces de noms avec des PVC portant le même nom dans différents espaces de noms, ce qui pouvait entraîner une perte de données.
...
IMPORTANT: Pour les clients utilisant SolidFire, veuillez ne pas effectuer la mise à jour vers la version 25.06.1 en raison d'un problème connu lors de la dépublication des volumes. La version 25.06.2 sera bientôt disponible pour corriger ce problème.
...
** Correction des volumes GCNV qui utilisaient par défaut NFSv3 lorsque  `nfsMountOptions` n'est pas défini ; désormais, les protocoles NFSv3 et NFSv4 sont pris en charge. Si  `nfsMountOptions` n'est pas fourni, la version NFS par défaut de l’hôte (NFSv3 ou NFSv4) est utilisée.
