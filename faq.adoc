...
Oui, Trident peut être installé hors ligne. Reportez-vous à link:../trident-install/kubernetes-deploy.html["En savoir plus sur l'installation de Trident"^].
...
Vous pouvez générer et modifier localement les fichiers manifestes exacts utilisés par Trident pour l'installation, si nécessaire. Reportez-vous à link:trident-install/kubernetes-customize-deploy-tridentctl.html["Personnalisez l'installation tridentctl"^].
